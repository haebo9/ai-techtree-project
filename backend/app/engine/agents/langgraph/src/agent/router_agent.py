...
2. NEXT_QUESTION: 현재 문제를 넘기고 다음 문제 요청 (예: "패스", "다음", "다른 문제 줘")
3. CHANGE_TOPIC: 면접 주제를 변경하길 원함 (예: "자바로 할래", "DB 질문 줘")
4. CONSULT: 면접 문제 풀이가 아닌 모든 일반 대화, 정보 검색 요청, 질문 (예: "LangGraph 정보 찾아줘", "이게 뭐야?", "공부법 알려줘", "안녕")
5. QUIT: 인터뷰 종료 요청 (예: "그만", "종료")

[주의사항]
- 사용자가 답을 시도했다면 무조건 ANSWER입니다.
- "찾아줘", "알려줘", "검색해줘" 등 정보 요청은 CONSULT로 분류하세요.
