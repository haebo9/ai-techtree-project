...
export default function CompletePage() {
  ...
  return (
    <main className="theme-page min-h-screen px-4 py-12 flex items-center justify-center">
      <div className="theme-shell w-full max-w-2xl rounded-2xl border p-8 sm:p-12 text-center">
        ...
        <p className="mb-3 text-xs font-black uppercase tracking-[0.22em] text-[#B88A3A]">Interview Completed</p>
        <h1 className="text-3xl sm:text-4xl font-black text-[#17232B] tracking-tight mb-4">면접이 종료되었습니다</h1>
        <p className="text-[#243844] font-bold leading-relaxed mb-8">
          분석 리포트는 비동기로 생성된 뒤 입력하신 이메일로 전송됩니다.
          {email && (
            <span className="block mt-2 text-[#4556D6]">{email}</span>
          )}
        </p>

        <div className="rounded-2xl bg-[#EAF4F7]/70 border border-[#B7C3CA]/55 p-5 text-left mb-8">
          <p className="text-sm font-black text-[#4556D6] mb-1">잠시만 기다려 주세요</p>
          <p className="text-sm font-medium text-[#243844] leading-relaxed">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <Link
            ...
            className="theme-cta rounded-2xl px-5 py-4 text-white text-base font-black transition-all active:scale-95"
          >
            같은 정보로 다시 연습하기
          </Link>
          <Link
            ...
            className="theme-silver-pill rounded-2xl border px-5 py-4 text-[#17232B] text-base font-black transition-all hover:bg-white/70 active:scale-95"
          >
            새 정보로 시작하기
          </Link>
        </div>
