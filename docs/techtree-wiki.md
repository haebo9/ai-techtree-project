...
- `/debug`에서 실제 프롬프트 diff와 주입 지침 추적을 강화한다.
- 배포 서버에서 certbot 갱신, 로그 로테이션, 헬스체크, 백업 정책을 자동화한다.
- 평가 리포트의 정량/정성 기준을 더 명확히 하고, 면접 모드별 rubric을 분리한다.

TechTree의 방향은 “AI가 면접을 대신 봐주는 서비스”가 아니라 “사용자가 자신의 답변을 실제 대화 속에서 검증하고 개선할 수 있게 돕는 면접 연습 환경”이다. 시스템 구조도 이 목적에 맞춰 입력 맥락, 실시간 대화, 평가 리포트, 운영 지침 개선 루프를 하나의 제품 경험으로 연결한다.
