...
### 5. 왜 채용 공고 추천을 LLM 생성에 맡기지 않는가?

**결정**: `Tavily 검색 결과 및 saved_jobs 강제 주입`

**근거**: 채용 공고는 실제 모집 현황과 정확한 URL 정보가 중요하므로 LLM의 환각(Hallucination) 위험을 배제해야 합니다. 따라서 평가 모델은 구조만 잡고, 시스템이 검색 API(Tavily)를 통해 수집한 검증된 데이터만 강제로 주입하도록 설계했습니다.
...
- 면접 중 추천 공고는 LLM이 지어내지 않고, Tavily 검색 결과 또는 사용자가 제공한 공고에서만 주입합니다.
- Realtime 프롬프트는 `short`와 `long`의 면접 목적 차이를 유지하고, reflection/policy는 이를 보조하는 운영 지침으로만 반영합니다.
- 최종 리포트의 `job_recommendations`는 모델 응답을 버리고, 시스템이 수집한 실제 공고 데이터로 강제 주입합니다.
